...
@DataJpaTest
@ActiveProfiles("test")
@Import({ com.tritech.tricore.shared.config.TestJpaConfig.class, ValidationTestConfig.class })
class UserRepositoryPortTest {
	...
	class IntegrityConstraintTest {
		...
		void testFullnameNotNullViolation() {
			User user = User.builder().subject(1L).email("user@example.com").build(); // manca
																						// fullname
		...
		void testEmailNotNullViolation() {
			User user = User.builder().subject(2L).fullname("Mario Rossi").build(); // manca
																					// email
		...
		void testSubjectUniquenessViolation() {
			User user1 = User.builder().subject(3L).fullname("Mario").email("mario@example.com").build();
	...
	class CrudTest {
		...
		void testSaveAndRetrieveUser() {
			...
			Optional<User> retrievedUserOpt = userRepositoryPort.findById(123456789L);
			...
			assertEquals("https://example.com/profile.jpg", retrievedUser.getPicture());
		}
		...
		void testUpdateUser() {
			...
			// correttamente
			assertEquals("Giuseppe Verdi Modificato", updatedUser.getFullname());
			assertEquals("nuovo.email@example.com", updatedUser.getEmail());

			// Doppia verifica: recupero nuovamente l'utente dal database
			Optional<User> retrievedUserOpt = userRepositoryPort.findById(987654321L);
			assertTrue(retrievedUserOpt.isPresent());
			assertEquals("Giuseppe Verdi Modificato", retrievedUserOpt.get().getFullname());
		}
	...
	class ConcurrencyTest {
		...
		void testOptimisticLocking() {
			...
			User user = User.builder().subject(123L).fullname("Alice").email("alice@example.com").build();
			userRepositoryPort.save(user);
	...
	class AuditFieldsTest {
		...
		void testCreatedAtAndUpdatedAtAreSet() {
			User user = User.builder().subject(20L).fullname("Luca").email("luca@example.com").build();
		...
		void testUpdatedAtChangesOnUpdate() throws InterruptedException {
			User user = User.builder().subject(21L).fullname("Laura").email("laura@example.com").build();
