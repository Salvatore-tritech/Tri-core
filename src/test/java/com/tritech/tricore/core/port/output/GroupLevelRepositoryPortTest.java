...
public class GroupLevelRepositoryPortTest {
	...
	class CrudTest {
		...
		void testSaveAndRetrieveGroupLevel() {
			...
			TypedQuery<GroupLevel> query = entityManager
				.createQuery("SELECT gl FROM GroupLevel gl JOIN FETCH gl.group WHERE " + "gl"
						+ ".groupName = :groupName AND gl.levelName = " + ":levelName", GroupLevel.class);
		...
		void testFindAllGroupLevels() {
			...
			GroupLevel groupLevel1 = GroupLevel.builder().groupName("admin").levelName("superadmin").build();

			GroupLevel groupLevel2 = GroupLevel.builder().groupName("user").levelName("entry user").build();

			GroupLevel groupLevel3 = GroupLevel.builder().groupName("user").levelName("mid user").build();
			...
			assertTrue(allGroupLevels.stream()
				.anyMatch(gl -> gl.getGroupName().equals("admin") && Objects.equals(gl.getLevelName(), "superadmin")));
			assertTrue(allGroupLevels.stream()
				.anyMatch(gl -> gl.getGroupName().equals("user") && Objects.equals(gl.getLevelName(), "entry user")));
			assertTrue(allGroupLevels.stream()
				.anyMatch(gl -> gl.getGroupName().equals("user") && Objects.equals(gl.getLevelName(), "mid user")));
		}
		...
		void testDeleteGroupLevel() {
			...
			GroupLevel groupLevel2 = GroupLevel.builder().groupName("user").levelName("basic user").build();
	...
	class ReferentialIntegrityTest {
		...
		void testDeleteAndVerifyForeignKeyConstraint() {
			...
			GroupLevel groupLevel2 = GroupLevel.builder().groupName("admin").levelName("basic admin").build();
			...
			// Verifico che il gruppo esista ancora (il vincolo di integrità
			// referenziale è rispettato)
		...
		void testDeleteGroupCascadesToGroupLevels() {
			...
			// Importante: eseguo un flush e clear per sincronizzare con il
			// database
			entityManager.flush();
			entityManager.clear();

			// Verifica con query nativa per assicurarsi che i record siano
			// stati
			// eliminati
			Query nativeQuery = entityManager
				.createNativeQuery("SELECT COUNT(*) FROM group_levels WHERE group_name = " + ":groupName");
		...
		void testIntegrityConstraint_GroupMustExist() {
			...
			GroupLevel groupLevel = GroupLevel.builder()
				.groupName("nonexistent") // Riferimento a un gruppo che
				// non
				// esiste nel DB
				.levelName("admin")
				.build();

			// Azione e Verifica: tentare di salvare dovrebbe generare
			// un'eccezione
			assertThrows(ConstraintViolationException.class, () -> {
				groupLevelRepositoryPort.save(groupLevel);
				entityManager.flush(); // Forza l'esecuzione delle
				// operazioni SQL
			});
		}
	...
	class CustomMethodsTest {
		...
		void testFindByGroupName() {
			...
			GroupLevel groupLevel1 = GroupLevel.builder().groupName("multiLevel").levelName("entry").build();

			GroupLevel groupLevel2 = GroupLevel.builder().groupName("multiLevel").levelName("intermediate").build();

			GroupLevel groupLevel3 = GroupLevel.builder().groupName("multiLevel").levelName("advanced").build();
		...
		@Test
		void testFindByGroupNameWithNoLevels() {
			// Preparazione: creo un gruppo senza livelli
			Group group = Group.builder().groupName("emptyGroup").build();
			groupRepositoryPort.save(group);

			// Verifica: il repository dovrebbe restituire una lista vuota
			List<GroupLevel> foundLevels = groupLevelRepositoryPort.findByGroupName("emptyGroup");
			assertTrue(foundLevels.isEmpty());
		}
	...
	class ConcurrencyTest {
		...
		void testOptimisticLocking() {
			// Preparazione: creo il gruppo e il livello associato
			Group group = Group.builder().groupName("marketing").build();
			groupRepositoryPort.saveAndFlush(group); // Salvo il gruppo

			GroupLevel groupLevel = GroupLevel.builder().groupName("marketing").levelName("entry").build();
			groupLevelRepositoryPort.saveAndFlush(groupLevel); // Salvo il livello per
																// ottenere una versione
																// iniziale

			// Recupero due istanze del livello da simulare transazioni separate
			GroupLevel g1 = entityManager.find(GroupLevel.class, new GroupLevelId("marketing", "entry"));
			entityManager.detach(g1);
			GroupLevel g2 = entityManager.find(GroupLevel.class, new GroupLevelId("marketing", "entry"));
			entityManager.detach(g2);

			// Modifico e salvo il primo record: la versione sarà incrementata
			g1.setUpdatedAt(LocalDateTime.now());
			groupLevelRepositoryPort.saveAndFlush(g1); // OK, version incrementata

			// Modifico il secondo record e provo a salvarlo
			g2.setUpdatedAt(LocalDateTime.now());

			// Verifico che l'operazione fallisca a causa del conflitto di version
			assertThrows(ObjectOptimisticLockingFailureException.class, () -> {
				groupLevelRepositoryPort.saveAndFlush(g2); // Fallisce: versione non
															// aggiornata
			});
		}
	...
	class AuditTest {
		...
		@Test
		void testAuditFieldsAreSetAndUpdated() throws InterruptedException {
			// Preparazione: creo un gruppo e un livello
			Group group = Group.builder().groupName("testGroup").build();
			groupRepositoryPort.save(group);

			GroupLevel groupLevel = GroupLevel.builder().groupName("testGroup").levelName("level1").build();

			// Salvataggio iniziale
			groupLevel = groupLevelRepositoryPort.save(groupLevel);

			// Verifica: `createdAt` e `updatedAt` non sono nulli
			assertNotNull(groupLevel.getCreatedAt());
			assertNotNull(groupLevel.getUpdatedAt());
		}
