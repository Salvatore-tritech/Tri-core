...
/**
 * Represents a group entity within the system. This class is mapped to the "groups" table
 * in the database.
 * <p>
 * Each Group is uniquely identified by its name (groupName) which must be both unique and
 * non-null.
 * <p>
 * This class includes fields for:
 * <ul>
 * <li>Database versioning using an optimistic locking strategy through the version
 * field.</li>
 * <li>Audit tracking, specifically the `createdAt` and `updatedAt` timestamps which store
 * the record creation and last update times respectively.</li>
 * </ul>
 * <p>
 * The `Group` entity is referenced by other entities, such as `GroupLevel`, to establish
...
public class Group {
	...
	/**
	 * Represents the unique name of a group within the system. This field is mapped to
	 * the "group_name" column in the "groups" table.
	 */
	@Id
