 ...
 * <li>Relationships to the `User` and `GroupLevel` entities. These relationships define
 * associations with a user and a specific group-level combination.</li>
 * </ul>
 * <p>
 * The `Permission` class utilizes composite keys defined by the `PermissionId` class.
 */
@Entity
...
public class Permission {
	...
	/**
	 * Represents the unique identifier of a user associated with a permission. This field
	 * is a part of the composite primary key for the `Permission` entity and is used to
	 * establish a relationship with the `User` entity.
	 * <p>
	 * This field is mapped to the "subject" column in the "permissions" table and must
	 * not be null.
	 */
	@Id
	...
	/**
	 * Represents the name of the group associated with a permission. This field acts as a
	 * part of the composite primary key for the `Permission` entity and is mapped to the
	 * "group_name" column in the database table corresponding to the `Permission` entity.
	 * <p>
	...
	/**
	 * Represents the name of the level associated with a permission. This field is a
	 * composite key component used to uniquely identify a `Permission` entity in
	 * combination with `subject` and `groupName`.
	 * <p>
	 * It is mapped to the "level_name" column in the database and must not be null.
	 */
	@Id
	...
	/**
	 * Represents the association between a permission and a user entity in the system.
	...
	/**
	 * Represents the relationship between the `Permission` entity and the `GroupLevel`
	 * entity.
	 * <p>
	 * This field defines a many-to-one association with the `GroupLevel` entity, where
	 * multiple `Permission` records can be associated with a single `GroupLevel`. The
	 * association is established through the composite key of `GroupLevel`, comprising
	 * `group_name` and `level_name`.
	 * <p>
	 * The database columns `group_name` and `level_name` in the `Permission` table are
	 * used to reference the corresponding `group_name` and `level_name` columns in the
	 * `GroupLevel` table.
	 * <p>
	 * The association is lazy-loaded, meaning the `GroupLevel` entity data is fetched
	 * from the database only when it is explicitly accessed. This improves performance by
